...
if __name__ == '__main__':
    ...
    dataset_2 = get_dataset(real_json=config.real_path, generated_json=config.generated_path, subject=config.subject_2, root_path=config.img_path)
    ...
    identifier_2 = generate_identifier(config.identifier_len)

    preprocess_fn = functools.partial(preprocess, transform=transform, tokenizer=tokenizer, identifier=identifier)
    preprocess_fn_2 = functools.partial(preprocess, transform=transform, tokenizer=tokenizer, identifier=identifier_2)

    dataset.set_transform(preprocess_fn)
    dataset_2.set_transform(preprocess_fn_2)

    train_dataloader = torch.utils.data.DataLoader(dataset, batch_size=config.train_batch_size, shuffle=True,
                                                   collate_fn=collate_fn)
    train_dataloader_2 = torch.utils.data.DataLoader(dataset_2, batch_size=config.train_batch_size, shuffle=True,
                                                   collate_fn=collate_fn)
    ...
    eval_list = [s.replace('[V1]', identifier) for s in eval_list['compositional']]
    eval_list = [s.replace('[V2]', identifier_2) for s in eval_list['compositional']]
    ...
    unet, text_encoder, optimizer, train_dataloader, train_dataloader_2 = accelerator.prepare(
        unet, text_encoder, optimizer, train_dataloader, train_dataloader_2
    )
    ...
    for epoch in range(config.num_train_epoch):
        ...
        for step, (batch, batch_2) in enumerate(zip(train_dataloader, train_dataloader_2)):
